 ...
 */
export const HTMLTemplateRenderer: React.FC<HTMLTemplateRendererProps> = ({
  validationResults,
  onReportGenerated,
}) => {
  ...
  const replaceMustacheVariables = (template: string, data: any): string => {
    ...
    const translationVars = [
      ...
      't.skipped', 't.moreOfSameType', 't.moreElementsNotShown'
    ];
  ...
  const processSpecificationLoops = (template: string, data: any): string => {
    ...
    if (specTemplate && data.specifications) {
      ...
      const specHtml = data.specifications.map((spec: any) => {
        ...
        const specVars = ['name', 'description', 'instructions', 'status_text', 
  ...
  const processEntityTables = (template: string, req: any): string => {
    ...
    if (req.passed_entities && req.passed_entities.length > 0) {
      ...
      if (passedTemplate) {
        ...
        const entityRows = req.passed_entities.map((entity: any) => 
          `<tr>
            <td>${entity.type}</td>
        ...
        const tableHtml = tableContent.replace(
          /<tbody>[\s\S]*?<\/tbody>/,
    ...
    if (req.failed_entities && req.failed_entities.length > 0) {
      ...
      if (failedTemplate) {
        ...
        const entityRows = req.failed_entities.map((entity: any) => 
          `<tr>
            <td>${entity.type}</td>
            <td>${entity.predefinedType || ''}</td>
            <td>${entity.name}</td>
