...
export class IDSTranslationService {
  ...
  translateValidationResults(results: any): any {
    ...
    translated.specifications = results.specifications.map((spec: any) => 
  ...
  private getStatusText(status: boolean | string, language: string, isSkipped: boolean): string {
    if (isSkipped) {
      switch (language) {
        case 'de': return 'ÜBERSPRUNGEN';
        case 'fr': return 'IGNORÉ';
        case 'it': return 'SALTATO';
        case 'rm': return 'SURSIGLÌ';
    ...
    if (status) {
      switch (language) {
        case 'de': return 'BESTANDEN';
        case 'fr': return 'RÉUSSI';
        case 'it': return 'SUPERATO';
        case 'rm': return 'REUSSÌ';
        default: return 'PASS';
      }
    } else {
      switch (language) {
        case 'de': return 'FEHLGESCHLAGEN';
        case 'fr': return 'ÉCHEC';
        case 'it': return 'FALLITO';
        case 'rm': return 'BETG REUSSÌ';
        default: return 'FAIL';
      }
    }
  }
  ...
  private translateSpecification(spec: any, language: string): any {
    ...
    translated.applicability = spec.applicability.map((app: string) => 
      this.translateApplicabilityText(app, language)
    );

    // Translate requirements
    translated.requirements = spec.requirements.map((req: any) => 
  ...
  private translatePropertyApplicability(text: string): string {
    ...
    if (match) {
      ...
      if (value) {
        return this.t('report.ids.patterns.propertyWithValue', 
        ...
        return this.t('report.ids.patterns.propertyInSet', 
          'Property {{property}} in set {{propertySet}}', {
          property,
          propertySet: this.translatePropertySet(propertySet)
        });
      }
